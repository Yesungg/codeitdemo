...
import memoryData, { getMemoriesByGroupId } from "../data/memoryData";
...
const MemoryList = ({ groupId }) => { // 부모에서 값을 업데이트할 수 있도록 props 추가
  ...
  console.log("📌 필터링된 메모리 데이터:", memories);
  ...
  //   console.log("✅ memoryData는 배열입니다.", memoryData);
  //   console.log("✅ memoryData 첫 번째 요소:", memoryData.length > 0 ? memoryData[0] : "데이터 없음");
  // } else {
  //   console.error("❌ memoryData가 배열이 아닙니다!", memoryData);
  // }
  ...
  //  // ✅ 현재 그룹의 모든 추억 개수 계산
  //  const totalMemories = memories.filter(
